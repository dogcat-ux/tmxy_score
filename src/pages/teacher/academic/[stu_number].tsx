...
import { getStuScoreApi, semesterOneApi, yearOneApi } from '@/pages/teacher/model';
...
const Academic: React.FC = () => {
  ...
  const score = useSelector((state: RootState) => state.teacher.oneStudentScore);
  ...
  const semesterList = useSelector((state: RootState) => state.teacher.semesterListOne);

  useEffect(() => {
    dispatch(setStuNum(stu_number));
    dispatch(getStuScoreApi({
  ...
  return (
    <div>
      <NavBar onBack={back} style={{fontSize:12}}>学生个人成绩</NavBar>
      <CommonHeader gpaMsg={`总绩点：${gpa}`} handleSemesterConfirmProp={handleSemesterConfirm}
