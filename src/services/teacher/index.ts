...
//   });
// }
export async function getStuGpa(body?: API.GetStuGpa, options?: { [key: string]: any }) {
  let params = new FormData();
...
export async function getStuScore(stu_number: string, body?: API.GetStuScore, options?: { [key: string]: any }) {
  let params = new FormData();
  _.forIn(body, function(value, key) {
    params.append(key, value);
  });
